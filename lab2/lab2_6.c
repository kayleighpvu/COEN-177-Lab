/*
    ...
    Date: 09/28/2023
    Title: Lab2 – step 6
    Description: This program computes 5 processes which requires
    4 fork() system calls and each process will iterate a different
...
int main(int argc, char *argv[]) {
    ...
    if (pid1 > 0){
        ...
        if (pid2 > 0){ 
            ...
            if (pid3){
                // pid2 > 0 and pid3 > 0 (child process)
                for (i=0;i<10;i++) {
                    printf("This is the c4 process pid=%d, the parent process is ppid=%d\n",getpid(), getppid());
                    usleep(n5);
                }
            }
            else{
